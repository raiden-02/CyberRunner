...
The static evaluator still answers geometry, navigation, spawn, and LOS questions. The playtest answers what scripted runners do when those facts play out over time.
...
Standing-ground navigation from the static evaluator still applies.
...
This playtest-driven loop is separate from the two designers in the evaluation suites. Their prompts and tools are frozen and untouched.

Tools: the six map edits, `run_playtest`, `finish_design`.

`run_playtest` is read-only. It always uses seed `20260831` and 64 rollouts so before/after reports are paired. The model chooses when to query. An edit applies the requested change, then returns a fresh evaluator inspection.
...
The evaluator stays at `hard 0`, paths `12/12` both sides.
...
This loop is not scored against the one-shot designer or the evaluator-grounded agent. There is no held-out suite here.

The two suites that do score those arms are recorded separately, with frozen manifest hashes:

- Basic repair, manifest `arena-forge-p4-v1`: `6acb4b3274ec7d1bb06090f5342816737227a9855945558958bc3d29154282e2`
- Interaction, manifest `arena-forge-p4b-v1`: `0ad49258552c067ebf1117dacc37b0c02ce16505870e943ef33e60ef571faa39`

## Recorded live session

Map: `map-contract-smoke`. One OpenAI call sequence. Not part of the scored suites.
...
2. `add_solid` occluder at (-5.5, 1.5, -3.5) hx=2 hz=0.4. Id `occluder-1`. Evaluator hard 0.
3. `run_playtest` same seed. Ghost A/B 44/20. Concentration 0.438 → 0.688. Overcorrected.
4. `resize_solid` `occluder-1` hx 2 → 1.2. Evaluator hard 0.
...
The evaluator stayed at 0 hard failures with both sites reachable. `ghost-spawn-0` → `sentinel-spawn-0` was clear on the source map and is now blocked by `occluder-1`. The brief said do not open that LOS. The model closed it.

This is not a claim that the playtest loop beats either scored designer. It shows the agent can query the simulator, edit, and compare paired reports.

## Live design in the lobby

The lobby Forge screen runs this same playtest-driven agent behind `POST /api/arena-forge/design`. Jobs live in memory. They disappear on server restart. At most one live job at a time.
