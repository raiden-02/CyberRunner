import { openaiFunctionToAnthropicTool } from "./anthropic-tools.js";
...
import type { PlaytestAgentSession, PlaytestAgentStartInput } from "./playtest-agent.js";
...
export function createProductAgentSession(
  mode: ArenaGameMode,
  opts?: {
  ...
  const tools = productFunctionTools(mode);
  const instructions = productSystemPrompt(mode);
  const formatStart = (input: PlaytestAgentStartInput) =>
    formatProductStartMessage({ ...input, mode });

  if (config.provider === "anthropic") {
    return new AnthropicPlaytestAgentSession({
      model: config.model,
      apiKey: readAnthropicApiKey(env),
      tools: tools.map((t) => openaiFunctionToAnthropicTool(t)),
